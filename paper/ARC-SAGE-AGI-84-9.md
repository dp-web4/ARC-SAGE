...
- Claude Opus 4.6, 1M context, Anthropic (via Claude Code — solver and paper lead)
...
The retrieval mechanism itself is built on a neuromorphic memory architecture called `membot` (MIT licensed, <https://github.com/project-you-apps/membot>). The core claim is retrieval at O(1) cost regardless of memory size, enabling a small model to access arbitrarily large knowledge bases without degrading inference latency.
...
- **F1 (search / thalamus)**: the retrieval layer. Cosine similarity on Nomic embeddings, accelerated by sign-zero Hamming pre-filtering and keyword boosting. This is the production search path and what the Phase 2 agent invokes at each game step. R@1 = 1.0 at 1M patterns. Typical query latency: 2–10 ms warm, depending on corpus size.
...
- **Complementary Learning Systems (McClelland, McNaughton & O'Reilly, 1995)** — Hippocampus/neocortex split mapped onto membot's F1/F0 layers (§6.2).
- **LongMemEval (Wu et al., 2024)** — 500-question long-term conversation memory benchmark used to validate membot retrieval independent of game-playing (§6.2).

---

## 9. Contributions and Acknowledgments

**Dennis Palatov** (Metalinxx, Inc.) — Research direction, SAGE architecture, problem framing, experimental design.

**Claude Opus 4.6, 1M context** (Anthropic, via Claude Code) — Solver implementation, world-model derivation, multi-agent frame-questioning sessions, capture-replay pipeline, competition submission, this paper draft.

**Andy Grossberg** (Waving Cat Learning Systems) — Membot paired-lattice architecture, cartridge format and tooling, multi-cart / federated / multiuser (Membox) infrastructure, LongMemEval benchmark validation, session memory system enabling persistent cross-session Claude operation, overall Project-You research direction.
